...
<?php echo $navbar_content; ?>
    ...
    <?php if ($error) { ?>
    <div class="alert alert-danger" role="alert"><?php echo $error; ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="/settings">返回</a></div>
    <?php } else {?>
        <div class="panel panel-default">
          ...
          <div class="panel-body">
            <form action="/settings" method="post">
              ...
              <button type="button" class="btn btn-primary" onclick="update_config()">修改</button>
            </form>
          </div>
        </div>
    <?php } ?>
...
<?php if (!$error) { ?>
    ...
    function update_config() {
        ...
        $.post('/update_setting_config', {config:config_value}, function(result){
            // 判断是否有错误
            if (result.error) {
                alert(result.error);
            }
